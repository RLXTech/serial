...
serial::list_ports(void) {
    ...
    while ((serial_port = IOIteratorNext(serial_port_iterator))) {
        ...
        // 折中的方案，后续改进
        if (device_name == "GDAS_M")
            description = rtrim("GDAS-M");
        else if (device_name == "GDAS_R") {
            char last_char = device_path.back();
            if (last_char == '1') {
                description = rtrim("GDAS-R-PT");
            } else if (last_char == '3') {
                description = rtrim("GDAS-R-CL");
            }
        }
